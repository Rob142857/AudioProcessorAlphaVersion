...
class CustomDictionary:
    ...
    def apply_substitutions(self, text: str) -> str:
        ...
        for incorrect, correct in self.substitutions:
            ...
            if matches > 0:
                # Custom replacement function that handles case properly
                # Use default argument to capture 'correct' by value, not reference
                def smart_replace(m, replacement=correct):
                    matched = m.group(0)
                    # If the entire match is uppercase, keep replacement as-is (don't force uppercase)
                    # This fixes issues like "THE MUNJAKAI..." staying uppercase
                    if matched.isupper() and len(matched) > 3:
                        # For long ALL CAPS matches, use the correct case from dictionary
                        return replacement
                    elif matched.islower():
                        return replacement.lower() if replacement.islower() else replacement
                    elif matched.istitle():
                        return replacement.capitalize() if replacement[0].islower() else replacement
                    else:
                        return replacement
                
                modified_text = re.sub(pattern, smart_replace, modified_text, flags=re.IGNORECASE)
