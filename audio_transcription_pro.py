"""
Audio Transcription Pro - Modern GUI
Professional speech-to-text transcription with maximum hardware optimization.

Features:
- Modern dark theme interface
- Auto device detection (CUDA > DirectML > CPU)
- Maximum hardware utilization
- Large model with disabled guardrails
- No VAD, no preprocessing for continuous speech
...
def run_transcription(input_file: str, outdir: str, output_queue: queue.Queue):
    ...
    try:
        ...
        output_queue.put(f"🚀 Starting MAXIMUM PERFORMANCE transcription for: {os.path.basename(input_file)}\n")
...
def launch_gui(default_outdir=None):
    if tk is None:
    ...
    subtitle_label = ttk.Label(title_frame, text="Professional speech-to-text with maximum hardware optimization",
...
🛡️  AI Guardrails Disabled (Captures all audio)"""
    ...
    # Status labels
    progress_label = ttk.Label(progress_frame, text="Ready to start transcription", style='Modern.TLabel', font=('Segoe UI', 10, 'bold'))
...
def main():
    parser = argparse.ArgumentParser(description="Modern GUI for speech-to-text transcription")
