...
if (-not (Test-Path $mf) -or $Force) {
    ...
    if (-not (Invoke-Download $uri $mf)){
        ...
        Write-Host "Please open the following URL in a browser and download the appropriate installer, then re-run this script when the file is saved to: $mf"
...
try{
    ...
    Write-Warn "conda install failed or was interrupted. You can try running the command manually in a new shell."
}
...
Write-Host "[INFO] If you plan to install a specific PyTorch wheel, install it after activating the env." -ForegroundColor Cyan
